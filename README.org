* SMC Actuator Position Control via Modbus/RTU
  ...
  [[http://www.smcworld.com/actuator/en/gateway.jsp][Gateway Units]], providing access several protocols such as DeviceNet,
  EtherNet/IP and ProfiBus.

** SMC Actuator and Gateway Protocol

  The underlying protocol spoken by the SMC Actuators themselves (and all the
  SMC Gateways) is simply Modbus/RTU, over an RS-485 serial multi-drop network.

  Using =cpppo_positioner=, you can access multiple SMC electric actuators, and:
  - issue multiple positioning operations simultaneously
  - monitor any positioning operation for completion
  - set and clear any positioner outputs
  - monitor all status flags

  The =cpppo_positioner= module allows control of the position of a set of
  actuators by initiating an RS-485 communication channel and issuing new
  position directives via each actuator's on-board controller.  The current
  state is polled as necessary via Modbus/RTU reads, and data updates and state
